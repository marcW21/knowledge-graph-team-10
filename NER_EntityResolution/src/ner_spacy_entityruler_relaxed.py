...
1. runs spaCy ORG NER
2. adds an EntityRuler built from company_seed values
3. optionally adds seed names from an external txt list
4. adds simple suffix / punctuation / uppercase variants for better recall
...
LEGAL_SUFFIXES = [
    ...
    "Biotech", "Biotechnology", "Biosciences", "Sciences", "Research", "Holdings",
]
...
def load_seed_list(seed_list_file: str | None) -> List[str]:
    if not seed_list_file:
        return []
    path = Path(seed_list_file)
    if not path.exists():
        raise FileNotFoundError(f"Seed list file not found: {path}")
    seeds: List[str] = []
    with path.open("r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            seed = normalize_text(line)
            if is_reasonable_seed(seed):
                seeds.append(seed)
    return seeds
...
def seed_variants(seed: str) -> Set[str]:
    ...
    variants: Set[str] = {seed, seed.upper()}

    no_parens = normalize_text(PARENS_RE.sub("", seed))
    if no_parens and no_parens != seed:
        variants.add(no_parens)
        variants.add(no_parens.upper())

    punct_light = normalize_text(seed.replace("&", " and "))
    if punct_light:
        variants.add(punct_light)

    no_trailing_periods = normalize_text(seed.replace(".", ""))
    if no_trailing_periods:
        variants.add(no_trailing_periods)

    base_candidates = {seed, no_parens, no_trailing_periods}
    for base in list(base_candidates):
        base = normalize_text(base)
        if not base:
            continue
        for suffix in LEGAL_SUFFIXES:
            variants.add(f"{base} {suffix}".strip())

    return {v for v in variants if is_reasonable_seed(v)}


def build_entity_ruler_patterns(df: pd.DataFrame, extra_seeds: Iterable[str]) -> List[Dict]:
    patterns: List[Dict] = []
    seen: Set[str] = set()
    ...
    candidate_seeds.extend(list(extra_seeds))
    ...
    for seed in candidate_seeds:
        ...
        for variant in seed_variants(seed):
...
def add_entity_ruler(nlp: Language, df: pd.DataFrame, extra_seeds: Iterable[str]) -> Language:
    if "entity_ruler" in nlp.pipe_names:
        nlp.remove_pipe("entity_ruler")

    ruler = nlp.add_pipe("entity_ruler", before="ner", config={"phrase_matcher_attr": "LOWER", "overwrite_ents": False})
    patterns = build_entity_ruler_patterns(df, extra_seeds)
    if patterns:
    ...
    for (_, row), doc in zip(df.iterrows(), nlp.pipe(texts, batch_size=batch_size)):
        ...
        for ent in doc.ents:
            if ent.label_.upper() not in keep_labels:
                continue
            mention_source = "entityruler" if getattr(ent, "ent_id_", "") else "spacy_or_entityruler"
            records.append(
                {
                    **base,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--seed-list-file", default="", help="Optional txt file with one seed company per line")
    parser.add_argument(
        "--entity-labels",
        nargs="+",
        default=["ORG"],
...
def main() -> None:
    ...
    extra_seeds = load_seed_list(args.seed_list_file)
    nlp = load_spacy_model(args.spacy_model)
    nlp = add_entity_ruler(nlp, df, extra_seeds)
    ...
    if extra_seeds:
        print(f"[ner_spacy_entityruler] Loaded {len(extra_seeds)} extra seed(s) from: {args.seed_list_file}")
